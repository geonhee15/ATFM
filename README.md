...
<table>
  ...
  <tr>
    <td align="center" colspan="2"><img src="docs/screenshots/storyboard.png" width="300"><br><b>메모 · 스토리보드</b><br><sub>장면별 캔버스(도형·텍스트·스케치·이미지·비율) · 배경음악 타임라인에 장면 구간</sub></td>
    <td align="center"><img src="docs/screenshots/cctv.png" width="230"><br><b>퀵 CCTV</b><br><sub>iPhone·Android 폰 카메라 · 움직임 감지 (샘플 영상)</sub></td>
  </tr>
</table>
...
- 설정: 최대 보관 개수, 중복 항목 위로 올리기, 이미지/파일 저장 여부, 로그인 시 자동 실행,
  **테마**(ATFM 기본 · JARVIS · 미드나이트 · 세이지 — 말풍선/미니 플레이어의 강조색·틴트·외관).
  맨 아래 **정보** 카드에 버전 · 개발자(geonhee15) · 소스 링크 · GitHub / 피드백 남기기 / 개발자 프로필 버튼
- **체크리스트** 탭: 적고 Enter로 추가, 체크로 완료, 더블클릭 수정, 호버 ✕ 삭제, 완료 항목 접기/한 번에 지우기.
  ...
  - **보관함**: 날짜가 바뀌면 전날 목록(완료·미완료 모두)이 그 날짜 아래로 자동 보관(앱이 꺼져 있었으면 다음 실행 때).
    ...
    마감일이 오늘 이후로 잡힌 미완료 항목은 날짜가 바뀌어도 보관하지 않고 마감까지 목록에 남김(마감이 지나면 다음 날 보관)
...
- **메모** 탭 (상단 세그먼트로 두 섹션)
  ...
  - **스토리보드**: 영상 기획용. 스토리보드를 여러 개 두고(이름 바꾸기·삭제), 각 스토리보드에 **Scene 1, 2, 3…** 을 추가/복제/순서 바꾸기/삭제.
    ...
    **이미지**(Finder 파일이나 브라우저의 그림을 캔버스에 **끌어다 놓기**, 또는 추가 › 이미지 파일…; 투명도, 원본 비율로)를
    올려 드래그로 옮기고 모서리 핸들로 크기를 바꿈. 이미지는 `storyboard-images/`에 복사본을 둠. 장면 아래 한 줄 메모(대사·카메라). 좌표는 비율로 저장해서 화면 비율을 바꿔도 그대로 따라옴
  - **배경음악**: 오디오 파일을 고르면 파형 타임라인이 생기고 장면들이 구간을 나눠 가짐(기본 균등). 구분선을 끌어서 "여기부터 여기까지 Scene 2"처럼
    ...
    ⋯ › **사용 구간 정하기**로 곡의 일부(예: 0:04–0:34)만 쓰도록 양쪽 핸들을 끌면 타임라인·재생·장면 구간이 그 범위 안에서만 움직임
...
- **날짜** 탭
  ...
  - **전용 달력**: 월 이동, 날짜 클릭으로 그날 일정 보기, 더블클릭 또는 "일정 추가"로 새 일정(제목 · 날짜 · 시간 · 메모 · 색 7종).
    "기간" 옵션으로 며칠부터 며칠까지 이어지는 일정은 달력에 가로 막대로 이어져 표시되고, D-day에서는 진행 중일 때 "N일째 · 끝까지 D-N"
...
- **타이머** 탭: **스탑워치**(0.01초, 랩 기록·구간 시간, 스페이스로 시작/정지)와 **타이머**(1분~1시간 프리셋 + 시/분/초 조절, 일시정지·계속·+1분).
  타이머가 돌면 메뉴 막대 아이콘 옆에 남은 시간이 표시되고, 끝나면 소리 + 화면 위쪽 팝업 + macOS 알림(첫 실행 때 알림 권한 요청).
  **종료 소리**는 macOS 내장 소리 14종(Glass · Ping · Hero …) 중 고르거나 내 오디오 파일을 지정, 미리 듣기와 반복 횟수(1~5번) 설정.
  말풍선을 닫아도 계속 돕니다
- **사전** 탭: 영어 · 국어 · 주기율표
  ...
  - **주기율표**: 118개 원소를 한 화면에(족별 색, 위에 족 1~18 · 왼쪽에 주기 1~7 라벨, 란타넘족 * · 악티늄족 ** 줄).
    이름(한글·옛 이름·영어)·기호·번호로 검색, 원소를 누르면
    원자량 · 주기/족/블록 · 실온 상태 · 전자 배치 · 전기음성도 · 밀도 · 녹는점/끓는점 · 발견 + **대표 화합물 3개**(H → H₂O 물 · CH₄ 메테인 · NH₃ 암모니아) + 위키백과 링크.
...
- **퀵 CCTV** 탭
  - **iPhone을 CCTV로**: 연속성 카메라(iOS 16+, 같은 Apple 계정, Wi-Fi·블루투스)를 켜고 폰을 잠근 채 가로로 세워 두면 목록에
    "OO의 iPhone 카메라"가 나타나고, 시작을 누르면 실시간 영상이 말풍선 안에 보임(AVFoundation, 첫 실행 때 카메라 권한).
    Mac 내장 카메라·USB 카메라도 같은 목록에서 선택
  - **Android 폰 · 멀리 있는 폰**: 폰에 IP 카메라 앱(Android는 IP Webcam, iPhone은 DroidCam 등)을 깔고 그 앱의 MJPEG 주소
    (`http://192.168.0.12:8080/video` 같은)를 넣으면 같은 Wi-Fi에서 그 스트림을 표시. 집 밖에서는 앱의 터널 기능이나 Tailscale 같은 VPN으로
  ...
  - **스냅샷**(JPEG) · **녹화**(.mov, 카메라 모드) · **플로팅 창**(항상 위, 크기·위치 기억)
- **파일 변환 · 다운로드** 탭
...
- **미니 플레이어** 탭: Spotify(앱·웹 플레이어)나 다른 앱이 노래를 재생하면 화면 구석에 작은 플레이어가 뜹니다
  ...
  - 재생 버튼 아래 **시스템 볼륨 슬라이더**(Core Audio 출력 장치 볼륨) + 스피커 아이콘으로 음소거. 볼륨 키로 바꿔도 즉시 따라옴
  - 아티스트 이름 옆 5칸 막대 **비주얼라이저**: 실제 시스템 오디오를 Core Audio 프로세스 탭(macOS 14.2+)으로 읽어 5개 대역
    ...
    미니 플레이어 탭의 "소리에 반응하는 비주얼라이저"로 끌 수 있음
...
| `ATFM_DEBUG_DATES_SELECT=2026-09-25` · `ATFM_DEBUG_DATES_SEARCH=키워드` | 날짜 탭에서 해당 날짜를 선택한 채 · 검색어를 채운 채 시작합니다 (스냅샷용) |
...
| `ATFM_DEBUG_CCTV_SAMPLE=1` | 퀵 CCTV를 생성한 샘플 영상(6초마다 지나가는 방문자)으로 시작하고 움직임 감지를 켭니다 (설정은 저장하지 않음); `ATFM_PROBE_CCTV=1 ATFM --probe`는 보이는 카메라와 권한 상태 출력 |
...
├── CCTV/        퀵 CCTV (CCTVMonitor: AVCaptureSession 카메라/연속성 카메라 · MJPEGClient · MotionDetector 격자 차분 · 녹화 · 플로팅 창)
