...
- **퀵 CCTV** 탭
  ...
  - **Android 폰 · 멀리 있는 폰**: 폰에 IP 카메라 앱(Android는 IP Webcam, iPhone은 DroidCam 등)을 깔고 그 앱의 MJPEG 주소
    ...
    끊기면 마지막 화면을 유지한 채 3·5·8·12·15초 간격으로 자동 재연결(시작을 끄기 전까지). 프레임 디코딩(ImageIO)과 움직임 계산은
    백그라운드에서 하고 표시는 최대 15fps로 제한해 1080p 30fps 스트림에도 말풍선이 버벅이지 않음. 소켓은 열려 있는데 프레임이 안 오면
    3초 뒤 "영상 멈춤" 표시, 12초면 다시 연결
